...
import arrow
app = Flask(__name__)
app.debug = False # Make this true if you want to turn on debug mode
...
def lab_env_db_avg():
	...
	if isinstance(range_h_int,int):	
		...
		time_from 		= time_now - datetime.timedelta(days = range_h_int)
		time_to   		= time_now
	...
	import sqlite3
        ...
        curs=conn.cursor()
        curs.execute("SELECT avg(temp) FROM temperatures WHERE rDateTime BETWEEN ? AND ?",(from_date_str,to_date_str))
        temperatures = curs.fetchone()[0]
        curs.execute("SELECT avg(temp) FROM humidities WHERE rDateTime BETWEEN ? AND ?",(from_date_str,to_date_str))
        humidities = curs.fetchone()[0]
        curs.execute("SELECT max(temp) FROM temperatures WHERE rDateTime BETWEEN ? AND ?", (from_date_str,to_date_str))
        maxtemperature = curs.fetchone()[0]
        curs.execute("SELECT max(temp) FROM humidities WHERE rDateTime BETWEEN ? AND ?", (from_date_str,to_date_str))
        maxhumidity = curs.fetchone()[0]
        curs.execute("SELECT min(temp) FROM temperatures WHERE rDateTime BETWEEN ? AND ?", (from_date_str,to_date_str))
        mintemperature = curs.fetchone()[0]
        curs.execute("SELECT min(temp) FROM humidities WHERE rDateTime BETWEEN ? AND ?", (from_date_str,to_date_str))
        minhumidity = curs.fetchone()[0]
        conn.close()
        return render_template("lab_env_db_avg.html",temp=temperatures, hum=humidities, tmax=maxtemperature, hmax=maxhumidity, tmin=mintemperature, hmin=minhumidity, date=today, gdate=range_h_form)
